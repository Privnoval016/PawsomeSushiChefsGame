...
public class FreezeTower : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (target != null)
        {
            ...
            if (lastFire >= fireRate)
            {
                ...
                targetScript.isFrozen = true;
                targetScript.hasPlayedFreezeEffect = false;
                targetScript.freezeDuration = freezeDuration;
                targetScript.freezeEffect = freezeEffect;
    ...
    public GameObject FindClosestDistance(string tag, float maxDistance)
    {
        ...
        {
            if (!obj.GetComponent<EnemyMovementWaypoint>().isFrozen)
            {
